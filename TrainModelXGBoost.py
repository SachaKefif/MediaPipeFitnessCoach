...
print(f"Size of dataset label 3 : {len(df_3)}")
...
model = xgb.XGBClassifier(
    ...
    n_estimators=10000,                # Number of trees to build.
                                      # More trees usually increase accuracy
                                      # but also training time.

    learning_rate=0.001,                # How much each tree corrects the previous ones.
...
# 7. Convert the file into a .onnx file, for android
import onnxmltools
from onnxmltools.convert.common.data_types import FloatTensorType

# 1. Load trained XGBoost model
xgb_model = xgb.XGBClassifier()
xgb_model.load_model("fitness_coach_xgboost.json")

# 2. Tell ONNX what shape the data is (number of output, numbner of features)
...
onnxmltools.utils.save_model(onnx_model, "fitness_coach_onnx.onnx")

print(f"Successfully converted to ONNX! Expected input shape: [None, {number_of_features}]")